...
There are some missing values in names as well as iso2 compared to no missing values in iso3. Names have more missing values then iso2.
...
gm %>% 
  ...
  ggplot(mapping = aes(time, co2, col=name)) + geom_line()
```
...
gm %>% 
  ...
  group_by(region, time) %>% 
  summarize(avgco2 = mean(co2_PC)) %>% 
  ggplot(mapping = aes(time, avgco2)) + geom_col(col = time)
```
